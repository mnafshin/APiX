...
// waitForTransaction polls the engine until at least n transactions are stored
// or the timeout elapses, then returns the full list.
func waitForTransactions(t *testing.T, eng interface {
DB() interface {
ListTransactions(limit, offset int, method, url string, status int, search string) (interface{}, interface{}, error)
}
}, n int) {
t.Helper()
// This helper is intentionally simple; tests in this file use eng directly.
}
